...
public class DialogueVertexAnimator {
    ...
    public Action<string> setPortraitFunction;
    public Action<AudioClip> playAudioFunction;
    public DialogueVertexAnimator(TMP_Text _textBox, Action<string> _setPortraitFunction, Action<AudioClip> _playAudioFunction) {
        textBox = _textBox;
        setPortraitFunction = _setPortraitFunction;
        playAudioFunction = _playAudioFunction;
    }
    ...
    public IEnumerator AnimateTextIn(List<DialogueCommand> commands, string processedMessage, AudioClip voice_sound, Action onFinish) {
        ...
        float elapsedTime = 0;
        while (elapsedTime < 0.175f && !stopAnimating) {
        ...
        while (true) {
            ...
            for (int j = 0; j < charCount; j++) {
                ...
                if (charInfo.isVisible) //Invisible characters have a vertexIndex of 0 because they have no vertices and so they should be ignored to avoid messing up the first character in the string whic also has a vertexIndex of 0
                {
                    ...
                    //toPrint += "Char: " + charInfo.character + " char size: " + charSize + " framecount: " + Time.frameCount + Environment.NewLine;
            ...
            if (command.position == visableCharacterIndex) {
                switch (command.type) {
                    ...
                    case DialogueCommandType.Sound:
                        PlaySound(command.stringValue);
                        break;
                    case DialogueCommandType.Portrait:
                        setPortraitFunction?.Invoke(command.stringValue);
                        break;
                }
                commands.RemoveAt(i);
                i--;
    ...
    private const float WAVE_MAGNITUDE_ADJUSTMENT = 0.04f;
    ...
    private void PlayDialogueSound(AudioClip voice_sound) {
        if (Time.unscaledTime - lastDialogueSound > timeUntilNextDialogueSound) {
            ...
            playAudioFunction?.Invoke(voice_sound);
        }
    }

    private void PlaySound(string soundId) {
        string path = "Sounds/" + soundId;
        AudioClip sound = ResourceManager.LoadResource<AudioClip>(path);
        if (sound == null) {
            Debug.LogError("Sound file not found: " + path);
        } else {
            AudioManager.Instance.PlaySFX(sound, 1);
        }
    }
    ...
    public void HandleNonTextCommandsOnly(List<DialogueCommand> commands) {
        for (int i = 0; i < commands.Count; i++) {
            DialogueCommand command = commands[i];
            switch (command.type) {
                case DialogueCommandType.Sound:
